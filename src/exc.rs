...
fn malloc(ctx: &mut wr::Ctx, size: u32) -> Option<u32> {
    ...
    let freelist_val = follow(memory, freelist_ptr);
    if freelist_val == 0 {
        Some(alloc(memory, size))
    } else {
        let next_ptr = follow(memory, freelist_val);
        write(memory, freelist_ptr, next_ptr);
        Some(freelist_val)
    }
}

fn free(ctx: &mut wr::Ctx, size: u32, ptr: u32) {
    let memory = ctx.memory(0);
    let freelist_ptr = freelist_loc(size);
    let current_freelist_val = follow(memory, freelist_ptr);
...
fn alloc(memory: &wr::Memory, size: u32) -> u32 {
    ...
    let old_alloc_size = follow(memory, ALLOC_SO_FAR_PTR);
...
fn follow(memory: &wr::Memory, ptr: u32) -> u32 {
