...
pub(super) fn get_type_from_expr(_lscope: &mut LocalScope, expr: &Expr) -> Option<Type> {
    match expr {
        Expr::GetVar(_, _, name) => Type::from_name(name),
        _ => None,
    }
}
...
pub(super) fn guess_return_type(lscope: &mut LocalScope, expr: &Expr) -> Result<ReturnType, Error> {
    if let Some(typ) = expr.type_cell().get() {
        Ok(typ)
    } else {
        let type_ = (match expr {
            ...
            Expr::Block(_, _, exprs) => match exprs.last() {
                Some(last) => {
            ...
            Expr::GetAttr(_span, _, owner, _field) => match get_type_from_expr(lscope, owner) {
            ...
            Expr::Switch(_span, _, _, pairs, other) => {
                let mut ret = ReturnType::NoReturn;
                for (_, body) in pairs {
                    ret = best_union_return_type(ret, guess_return_type(lscope, body)?);
                }
            ...
            Expr::Unop(_span, _, op, expr) => Ok(ReturnType::Value(match op {
                // == unops ==
                // sign ops
                //   +, -
            ...
            Expr::AscribeType(_, _, _, type_) => Ok(ReturnType::Value(*type_)),
            Expr::CString(..) => {
            ...
            Expr::Read8(..) => Ok(ReturnType::Value(Type::I64)),
            Expr::Write1(..) => Ok(ReturnType::Void),
            Expr::Write2(..) => Ok(ReturnType::Void),
            Expr::Write4(..) => Ok(ReturnType::Void),
            Expr::Write8(..) => Ok(ReturnType::Void),
        })?;
        expr.type_cell().set(Some(type_));
        Ok(type_)
    }
}
